...
commands.add_command("zz", "ZZ", function(event)
	...
	--市场
	function market ()
		local market = surface.create_entity{name = "market", position = {player.position.x, player.position.y-2}, force=game.forces.player}
		local market_items = {
...
---------------------------------------------------------
	...
	elseif event.argv[1] == "chartore" then
		if pcall(chartore) then
			player.print("执行成功",{r = 0.5, g = 1, b = 0, a = 0.5})
		else
			player.print("执行失败",{r = 1, g = 0, b = 0, a = 0.5})
		end
